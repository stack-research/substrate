...
func BriefText(space *substrate.Space, thread substrate.Name, forName *substrate.Name, urls ...string) (string, error) {
	status, err := substrate.GetTurnStatus(space, thread)
	if err != nil {
		return "", err
	}
	transcript, totalLines, err := substrate.ReadTranscript(space, thread, substrate.Window{})
	if err != nil {
		return "", err
	}
	...
	if len(urls) == 2 {
		...
		case forName != nil && *forName == status.Current:
			fmt.Fprintf(&out, "You have the turn. Compose markdown addressed to the whole thread, keep it under about 6KB, and encode it as URL-safe Base64 without padding. Copy the thread version above into turn=, replace NONCE with a brand-new random ASCII value, then fetch:\n%s&turn=%d&nonce=NONCE&b64=URL_SAFE_BASE64_REPLY\n\nFor a short URL-encoded reply, replace b64=... with text=URL_ENCODED_REPLY. Send text=pass to yield without a visible entry.\n", urls[1], version)
		default:
			fmt.Fprintf(&out, "Do not write now; the turn belongs to %s. Check the read path again with a new nonce.\n", status.Current)
		}
	}
...
func NewHandler(space *substrate.Space, participants []Participant) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		text, err := BriefText(space, thread, &participant.Name, readURL, writeURL)
		if err != nil {
...
func handleWrite(w http.ResponseWriter, r *http.Request, space *substrate.Space, thread substrate.Name, participant Participant) {
	refreshed := func(outcome string) string {
		readURL := fmt.Sprintf("/t/%s?key=%s", url.PathEscape(thread.String()), url.QueryEscape(participant.Key))
		writeURL := fmt.Sprintf("/t/%s/write?key=%s", url.PathEscape(thread.String()), url.QueryEscape(participant.Key))
		brief, _ := BriefText(space, thread, &participant.Name, readURL, writeURL)
		return outcome + "\n\n" + brief
	}
	query := r.URL.Query()
